...
{  int m,i,jt,k,is,it;
   ...
   void g90();
   m=n;
   while ((m>0)&&(fabs(a[m])+1.0==1.0)) m--;
   if (m<=0) {printf("fail\n");return (-1);}
   for (i=0;i<=m;i++) a[i]=a[i]/a[m];
   for (i=0;i<=m/2;i++) {w=a[i];a[i]=a[m-i];a[m-i]=w;}
   k=m;is=0;w=1.0;
   jt=1;
   while (jt==1)
   {  pq=fabs(a[k]);
      while (pq<1.0e-12)
      {  xr[k-1]=0.0;xi[k-1]=0.0;k--;
	 if (k==1)
...
{  if (*it==0)
   ...
   {  *c=*c+(*dc);
      ...
      *x1=*x+*dx;*y1=*y+*dy;
      if (*c<=6.29) {*it=0;return;}
      *dd=*dd/1.67;
      if (*dd<=1.0e-07) {*it=1;return;}
      *c=0.0;
   }
}
